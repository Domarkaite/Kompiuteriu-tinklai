...
void forwardToClient1(int clientSocket, char *message) {
...
void forwardMessageToServer1(char *message) {
    ...
    clientAddr.sin6_port = htons(10004);
...
void *handleClient1(void *arg) {
    ...
    if (bytesRead > 0) {
        buffer[bytesRead] = '\0'; 
        printf("Server 1 received from Client 1: %s\n", buffer);

        if (ntohs(((struct sockaddr_in6 *)arg)->sin6_port) == SERVER22_PORT) {
            forwardToClient1(clientSocket, buffer);
        } else {
...
int main() {
    ...
    printf("Server 1 is listening on port %d\n", SERVER1_PORT);
    ...
    printf("Server 1 is waiting for connections...\n");

    while (1) {
        clientSocket = accept(serverSocket, (struct sockaddr *)&clientAddr, &clientLen);

        if (clientSocket != -1) {
            printf("Accepted the connection from Client 1\n");

            pthread_t tid;
            int *clientSockPtr = malloc(sizeof(int));
            *clientSockPtr = clientSocket;
            pthread_create(&tid, NULL, handleClient1, (void *)clientSockPtr);
            pthread_detach(tid);
        }
        ...
        if (server2Socket != -1) {
            ...
            close(server2Socket);}
                ...
                printf("Accepted the connection from Client 111\n");
